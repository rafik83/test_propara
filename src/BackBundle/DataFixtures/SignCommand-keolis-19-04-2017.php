                                    ...
                                    for ($i = 0; $i < count($tab_search); $i++) {
                                        for ($j = $i + 1; $j < count($tab_search); $j++) {
                                            if ($tab_search[$i] == $tab_search[$j]) {
                                                ...
                                                $import->setCommentError('Ce Numéro de sécurité social' . ' ' . $tab_search[$i] . ' ' . 'existe déja  dans ce fichier');
...
//                                $output->writeln("clef securité sociale");
                                $idrhuser = $this->getContainer()->getParameter('client_id');
...
//                                $Current_User = $repUser->findOneBy(array('username' => $username));
//                                $Current_User = $repUser->findOneBy(array('username' => $username));
//                                $output->writeln(dump($Current_User));
//                                $Salary2 = $repSalary->findOneBy(array('user' => $Current_User)); //teste if num secu existe
...
//                                $Salary2 = $repSalary->findOneBy(array('numSecu' => $securite_sociale));
                                $Salary2 = $repSalary->findOneBy(array('numSecu' => $num_secu_cless));
                                if ($Company) {
...
//                                        }
                                        ...
                                        if ($Companyinsert) {
                                            ...
                                            $login = addslashes(trim($data[array_search('ss', $coldisplay)]));
//                                            $currentuser = $repUser->findOneBy(array('username' => $username));
                                            $currentuser = $repUser->findOneBy(array('username' => $login));
//                                            $securi_sociale = $numSecu . '' . $matri;
//                                             $existesalary = $repSalary->findOneBy(array('numSecu' => $securi_sociale));
                                            $existesalary = $repSalary->findOneBy(array('user' => $currentuser));
                                            if ($existesalary == null) {
...
//                                        
                                                ...
                                                $output->writeln($numSecu);  //. addslashes(trim($data[array_search('cless', $coldisplay)]));
                                                $output->writeln("cle secu  ");
                                                $output->writeln(addslashes(trim($data[array_search('cless', $coldisplay)])));  //. addslashes(trim($data[array_search('cless', $coldisplay)]));
//                                                $username = $this->randomUsername($prenom_nom, strlen($prenom_nom));
//                                                $output->writeln("prenom_nom");
//                                                $output->writeln($prenom_nom);
//                                                $output->writeln("username");
//                                                $output->writeln($username);
...
//                                                $import->setCommentError('salarié' . ':' . ' ' . $existesalary->getNom() . ' ' . $existesalary->getPrenom() . ' ' . 'déjà existant avec le n° de sécurité social:' . ' ' . $existesalary->getNumSecu() . '' . 'différent. Si salarié différent le rajouter manuellement avant import du fichier.' . '');
                                                $import->setCommentError('login' . '' . $numSecu . '' . 'existe déja');
                                                $output->writeln("login existe deja");
                                                $import->setProgress(99);
                                                $db->flush();
                                                die();
                                            }
                                    ...
                                    $import->setCommentError('Problème dans la structure du fichier, nom de societé' . ':' . ' ' . $nomentreprise . ' ' . 'invalide');
                                    $output->writeln("nom entreprise invalide");
                                    $import->setProgress(99);
                                    $db->flush();
                                    die();
...
//                $output->writeln('delete directory1');
        ...
        try {
            ...
            $output->writeln("l3");
//            $output->writeln("depots");
//            $output->writeln(var_dump($depots));
//            die('ici');
...
//                                $output->writeln(var_dump($preg_replace));
                                ...
                                if (is_object($salary)) {
                                    ...
                                    if (!$salary->getIsPaper()) {
                                        ...
                                        $output->writeln("salary is  paper");
                                        //check if the document contain one or two pages
//                                        $image = new \Imagick();
//                                        $image->pingImage($file);
//                                        $nbpages = $image->getNumberImages();
//                                        $pagename = 'indefini';
//                                        if ($nbpages == 1) {
//                                            $pagename = $nbpages . '_page';
//                                        } else {
//                                            $pagename = $nbpages . '_pages';
//                                        }
                                        //Log zip
                                        $logMu = new LogMu();
                                        ...
                                        $salariesListForPaper[] = $salary;
//                                        if ($nbpages == 1) {
//                                            $salariesPrint1p[] = $salary;
//                                        } elseif ($nbpages == 2) {
//                                            $salariesPrint2p[] = $salary;
//                                        }
                                    }
                                    $output->writeln("l27");
                                } else {
                    ...
                    if (is_dir($dir)) { // $dir  $printDir . $nom
//                        $bulletin_is_paper = false;
//                        $bulletin_is_paper = $salary->getIsPaper();
//                        $filenamezip = "";
//                        if ($bulletin_is_paper == true) {
                        $output->writeln("enter if is_dir for ziping");
                        // Get real path for our folder
                        //$nom . '-' . date('d-m-Y-H-i-s'); 
                        $filenamezip = "";
                        if (count($salariesListForPaper) > 0) {
                            $filenamezip = $nom . '-' . date('d-m-Y-H-i-s') . '.' . 'zip'; //date('d-m-Y-H-i-s') . '.' . 'zip';
                        }
                        // Initialize archive object
                        //http://stackoverflow.com/questions/39194849/recursiveiteratoriterator-include-directory-and-php-files-in-zip
                        ////http://satooshi.github.io/php-coveralls/api/class-Symfony.Component.Finder.Iterator.RecursiveDirectoryIterator.html
                        $zip = new \ZipArchive();
                        $zip->open($printDir . $filenamezip, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
                        $rootPath_for_papier = realpath($dir); // ici on donne le chemin des bulletin en papier
                        ...
                        foreach ($files as $name => $file) {
                            ...
                            if (!$file->isDir()) {
                                ...
                                if (strpos($relativePath, '_') !== false) {
                                    $matricule_salary = explode('_', $relativePath);
                                    $salaryId_forPaper = $matricule_salary[0];
                                } elseif (strpos($relativePath, '.') !== false) {
                                    $matricule_salary = explode('.', $relativePath);
                                    $salaryId_forPaper = $matricule_salary[0];
                                }

                                if ($salaryId_forPaper != "") {
                                    if (!is_null($depot->getCompany())) {
                                        $salary_for_paper = $repSalary->findOneBy(array('matricule' => $salaryId_forPaper, 'company' => $depot->getCompany()));
                                    } else {
                                        $salary_for_paper = $repSalary->findOneBy(array('matricule' => $salaryId_forPaper));
                                    }

                                    if (is_object($salary_for_paper)) {

                                        if ($salary_for_paper->getIsPaper() == true) {
                                            // Add current file to archive
                                            $output->writeln("<-----------------bulletin a zipper------------>");
                                            $output->writeln(var_dump($relativePath));
...
//                        $output->writeln("end foreach");
                        ...
                        $zip->close();
                        // Remove directory
//                        $output->writeln("Remove directory");
//                        $poDir = $printDir . $prefixe_zip_papier;
//                        $this->deleteDirectory($poDir);
//                        }
                        // to print depot
...
//                        }
                    ...
                    echo $recordId;
                    echo '$certSign';
//                    $output->writeln("dump certsigne");
//                    $output->writeln(var_dump($recordId));
//                    if ($to_archive === 1) {
//                        $certSign->archiveRecord($recordId);
//                        echo '$certSign-archiveRecord';
//
//                        echo 'fin traittement';
//                    }
                    //Remove all the extracted directory
//                    $this->deleteDirectory($dir);
//                    echo 'delete directory3';
//                    die('dump $recordId');
                    // Fin de traitement
                    if (count($muErrors) > 0) {
                        $depot->setError(true);
                ...
                $progress->finish();
//                $output->writeln("count salariesListForPaper");
//                $output->writeln(var_dump($salariesListForPaper));
