...
//                    $output->writeln("dir");
//                    $output->writeln(var_dump($dir));
//                    $output->writeln("FilesystemIterator + SKIP_DOTS");
//                    $tt = \FilesystemIterator::SKIP_DOTS;
//                    $output->writeln(var_dump($tt));
//
//                    $output->writeln("var dump dir");
//                    $output->writeln(var_dump($dir));
//                    $fi = new \FilesystemIterator($dir, \FilesystemIterator::SKIP_DOTS);//  \FilesystemIterator::SKIP_DOTS==> int(4096), $dir ==> "/zips/files/13/uid/"
//                    $output->writeln(var_dump($fi));
//                    die('ici');
                    $fi = new \FilesystemIterator($dir, \FilesystemIterator::SKIP_DOTS);
                    $app = new Application();
                    $progress = $app->getHelperSet()->get('progress');
...
//                                die;
                                ...
                                if (is_object($salary)) {
                                    if (!$salary->getIsPaper()) {
                                        ...
                                        $output->writeln("salary is not paper");
                                        ...
                                        // create the directory page if doesn't exist
                                        // $printDir = '/docs_sign/' . $this->getContainer()->getParameter('client_id') . '/toprint/';
                                        $globalDir = $printDir . $nom . '/' . $pagename . '/';
                                        $output->writeln("printDir");
                                        $output->writeln(var_dump($printDir));
                                        $output->writeln("nom");
                                        $output->writeln(var_dump($nom));
                                        $output->writeln("pagename");
                                        $output->writeln(var_dump($pagename));
                                        $output->writeln("globaldir");
                                        $output->writeln(var_dump($globalDir));
                                        ...
                                        $array_pdf_to_zip[$index_pdf_to_zip] = $fullname . '.pdf';
                                        $index_pdf_to_zip ++;

                                        // copy the file to the whole file
                                        copy($file, $globalDir . $fullname . '.pdf');
                                        $output->writeln("globalDir");
                                        $output->writeln(var_dump($globalDir));
                                        $output->writeln("file");
                                        $output->writeln(var_dump($file));
...
//                                        }
                    ...
                    if (is_dir($printDir . $nom)) {
                        $output->writeln("enter is_dir(print dir + nom");
                        //$nom = 'all';   
                        // Zip the file
                        $filenamezip = $nom . '-' . date('d-m-Y-H-i-s') . '.zip';
                        $source = $globalDir.$fullname . '.pdf';
//                        $output->writeln("tt");
//                        $output->writeln(var_dump($tt));
//                        die;
                        $destination = $printDir . $filenamezip;
//                        $output->writeln("array_pdf_to_zip");
//                        $output->writeln(var_dump($array_pdf_to_zip));

                        $this->Zip2($source, $destination, $array_pdf_to_zip);
                        // Get real path for our folder
//                        $rootPath = realpath($printDir . $nom);
                        // Initialize archive object
//                        $zip = new \ZipArchive();
...
//                            if (!$file->isDir()) {
////                                $output->writeln("enter if + foreach");
...
//                        $zip->close();
                        $output->writeln("l32");
//                        $this->zip($printDir.$nom, $printDir.$filenamezip);
