                            ...
                            if (is_numeric(addslashes(trim($data[array_search('cless', $coldisplay)])))) {
                                ...
                                if (array_search('ss', $coldisplay) !== false) {
                                    ...
                                    if (!empty($numSecu)) {
                                        ...
                                        if (!is_numeric($numSecu)) {
                                            $import->setCommentError('Vous devez saisir un numéro de sécurité sociale valide   pour le salarié' . ' ' . addslashes(trim($data[array_search('nom', $coldisplay)])) . ' ' . utf8_encode(addslashes(trim($data[array_search('prenom', $coldisplay)]))));
                                            $import->setProgress(99);
                                            $db->flush();
                                            die();
                                        }
...
//                                    die('cless');
                                    if (!empty($ClefSecu)) {
                                        $output->writeln("cless not empty");
                                        $output->writeln($ClefSecu);
                                        if (!is_numeric($ClefSecu)) {
                                            $import->setCommentError('Vous devez saisir un clé de sécurité sociale valide   pour le salarié' . ' ' . addslashes(trim($data[array_search('nom', $coldisplay)])) . ' ' . utf8_encode(addslashes(trim($data[array_search('prenom', $coldisplay)]))));
                                            $import->setProgress(99);
                                            $db->flush();
                                            die();
                                        }
                                ...
                                //
                                //$output->writeln(trim($data[array_search('bupapier', $coldisplay)]));
                                //$output->writeln(array_search('bupapier', $coldisplay));
                                //test is paper
                                $bupapier = -1;
...
//                                    }
                                ...
                                if ($bupapier == -1) {
                                    $output->writeln("bupapier n'existe pas dans array_search data");
                                    $import->setCommentError('Vous devez saisir le statut du bulletin en papier ou en démat  pour le salarié qui a le N° sécurité sociale' . ' ' . addslashes(trim($data[array_search('ss', $coldisplay)])));
                                    $import->setProgress(99);
                                    $db->flush();
                                    die();
                                }
...
//                                                $import->setCommentError('Cette Matricule' . ' ' . $tab_search_matricule[$i] . ' ' . 'existe plusieur fois dans ce fichier');
//                                                $output->writeln("meme matricule");
//                                                $import->setProgress(99);
//                                                $db->flush();
//                                                $trouve_matricule = 1;
//                                                $output->writeln($trouve_matricule);
////                                                $output->writeln("tab_search_matricule");
////                                                $output->writeln(var_dump($tab_search_matricule));
//                                                die();
//                                            }
//                                        }
////                                       
                                                    ...
                                                    if ($id_salary != NULL) {
                                                        ...
                                                        if ($NewCompany) {
                                                            if ($Salary_Update) {
                                                                ...
                                                                $Salary_Update->setEmailPro($emailpro);
                                                                $Salary_Update->setMatricule($matricule);
                                                                $num_secu_cless = $numSecu . '' . $ClefSecu;
...
//                                            die('insert + user not null');
                                            ...
                                            if ($user2) {
                                                ...
                                                $NewSalary->setEmailPro($emailpro);
...
//            $buLocation = $this->getContainer()->getParameter('kernel.root_dir') . '/../web/uploads/docs_sign/' . $this->getContainer()->getParameter('client_id') . '/unite/';
            ...
            $_array_file_pdf_unitaire = array();
            $_index_file_pdf_unitaire = 1;
            $folder_parent_name_bu = '';

            $_array_stock_file_to_zip = array();
            $_index_stock_file_to_zip = 1;
            $_array_stock_salary = array();
            $_index_stock_salary = 1;
...
//                    $output->writeln(var_dump($filename));
                    ...
                    ///docs_sign/13/unite/e58f0285f2d302a28a4aa5e181b77bc8.pdf
//                    $output->writeln("file");
...
//                            die('ici');
                            ...
                            $_array_file_pdf_unitaire[$_index_file_pdf_unitaire] = $file;
                            $_index_file_pdf_unitaire ++;


                            $_array_stock_file_to_zip[$_index_stock_file_to_zip] = $file;
                            $_index_stock_file_to_zip ++;


                            $_array_stock_salary['nom_salary'][$_index_stock_salary] = $salary->getNom();
                            $_array_stock_salary['prenom_salary'][$_index_stock_salary] = $salary->getPrenom();
                            $_index_stock_salary ++;

//                            $output->writeln("$folder_parent_name_bu");
//                            $output->writeln(var_dump($folder_parent_name_bu));
//                            die('ici');
...
//                
                ...
                if (count($_array_stock_file_to_zip) > 0) {

                    for ($i = 1; $i <= count($_array_stock_file_to_zip); $i++) {
                        ...
                        $folder_parent_name_bu = '';
                        $filenamezip = "";
                        $nom_salarly = $_array_stock_salary['nom_salary'][$i];
                        $prenom_salary = $_array_stock_salary['prenom_salary'][$i];
                        $folder_parent_name_bu = 'Bulletin' . '-' . 'unitaire' . '-' . $nom_salarly . '-' . $prenom_salary;
                        if (count($_array_file_pdf_unitaire) > 0) {

                            $folder_parent_name_bu = $folder_parent_name_bu . '-' . date('d-m-Y-H-i-s');
                            $output->writeln("<-----------------create folder_parent_name to zip------------>");
                            $output->writeln(var_dump($folder_parent_name_bu));
                            if ($folder_parent_name_bu != '') {
//                                $array = $_array_file_pdf_unitaire[$i] ;
//                                $output->writeln("dump array");
//                                $output->writeln(var_dump($array));
//                                die('ici');
                                $this->FolderToZipContainPdfFile(array($i => $_array_file_pdf_unitaire[$i]), $folder_parent_name_bu);
                            }
                            //2-zip folder
                            if (count($salariesListForPaper) > 0) {
                                $filenamezip = $folder_parent_name_bu . '.' . 'zip';
                                $output->writeln("<--------------filenamezip--------------->");
                                $output->writeln(var_dump($filenamezip));
                                $_path_folder_bu_to_zip = '/docs_sign/' . $this->getContainer()->getParameter('client_id') . '/toprint/' . $filenamezip;
                                // Create recursive directory iterator
                                $zip = new \ZipArchive();
                                $zip->open($_path_folder_bu_to_zip, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
                                $folder_contain_file_unitaire_to_zip = '/docs_sign/' . $this->getContainer()->getParameter('client_id') . '/toprint/' . $folder_parent_name_bu;
                                // Get real path for our folder
                                $rootPath_for_papier = realpath($folder_contain_file_unitaire_to_zip); // ici on donne le chemin des bulletin en papier
...
//                            $output->writeln(var_dump($filenamezip));
                            ...
                            if (count($_array_file_pdf_unitaire) > 0) {
                                ...
                                $targer_folder_bu_after_zip = $_cuurent_target . '/' . $folder_parent_name_bu;
                                $output->writeln("<-----------------name folder to delete------------>");
                                $output->writeln(var_dump($folder_parent_name_bu));
                                if (is_dir($targer_folder_bu_after_zip)) {
                                    $_bool_delete_folder_bu_zip = $this->Delete($targer_folder_bu_after_zip);
                                    if ($_bool_delete_folder_bu_zip) {
...
//
//                $output->writeln("id_depot");
//                $output->writeln(var_dump($depot->getId()));
//            }
//
//            die('icicicici');
                                    ...
                                    if (!$salary->getIsPaper()) {
                                        ...
                                        if (!is_null($docName)) {
                                            ...
                                            if (!is_null($sigName)) {
                                                ...
                                                // stop the process and save the error
                                                $muErrors[] = 'Impossible de signer  le doc ' . $docName;
                                                //Log zip
                                                $logMu = new LogMu();
